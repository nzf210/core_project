          ...
          <tbody>
            <tr v-for="(l, idx) in result.links.slice(0, 50)" :key="idx">
              ...
              <td class="action-cell">
                <button class="copy-btn" @click="copyText(l.url, `url-${idx}`)">
                  {{ copiedKey === `url-${idx}` ? '✅ Link Disalin' : '🔗 Copy Link' }}
                </button>
                <button class="copy-btn btn-token" @click="copyText(l.token, `tok-${idx}`)">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(c, idx) in result.codes.slice(0, 50)" :key="idx">
              ...
              <td>
                <button class="copy-btn" @click="copyText(c.code, `code-${idx}`)">
